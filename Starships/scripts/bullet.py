...
class Bullet(arcade.Sprite):
    ...
    def __init__(self, filename: str, origin, destination, scale: float = 1, image_x: float = 0, image_y: float = 0, image_width: float = 0, image_height: float = 0, center_x: float = 0, center_y: float = 0, repeat_count_x: int = 1, repeat_count_y: int = 1, flipped_horizontally: bool = False, flipped_vertically: bool = False, flipped_diagonally: bool = False, mirrored: bool = None, hit_box_algorithm: str = "Simple", hit_box_detail: float = 4.5):
    ...
    def update(self):
        # print(self.position)
        if self.center_x < 0 or self.center_x > GAME_WIDTH or self.center_y < 0 or self.center_y > GAME_WIDTH or self.collides_with_point(self.destination):
            self.kill()
        return super().update()
